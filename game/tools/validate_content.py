...
SELECTOR_STRINGS = {"self", "caster", "target", "summoner", "it", "across", "adjacent",
                    "eventSource", "eventTarget", "eventTargets", "eventAffected", "eventUnit", "eventCounterer"}
STATUS_KINDS = {"damage_reduction", "incoming_damage_mod", "incoming_heal_mod", "outgoing_damage_mod",
                ...
                "isolated", "untargetable", "elemental_essence", "cost_mod", "cost_currency_remap", "instant_cast", "silence", "income_suppressed", "paralysis",
                "stealth", "veiled", "cloak", "reveal", "taunt", "blind", "stun", "channeling", "dot", "regen",
                "heal_lock", "uncounterable", "coil_damage_bonus", "mark", "stack"}
EVENTS = {"damageDealt", "shieldDamaged", "shieldBroken", "shieldGranted", "shieldConsumed", "unitDied", "skillUsed", "skillDeclared",
...
def v_condition(c, path):
    ...
    elif "declaredTargetsSelf" in c or "eventTargetsFaction" in c or "eventHasTag" in c or "eventSkillId" in c or "eventStatusKind" in c or "eventStatusNameIncludes" in c or "eventSourceId" in c or "eventTeamIsSelf" in c or "eventSourceSummonedBySelf" in c or "eventRedirectedToSelf" in c or "eventHidden" in c or "chance" in c or "isNamed" in c:
        pass
    elif "and" in c:
...
def v_skill(s, path):
    ...
    for i, m in enumerate(s.get("costMods", []) or []):
        ...
        deltas = [k for k in ("magnitude", "genericDelta", "specificDelta") if k in m]
        if not deltas:
            err(mp, "costMod needs a magnitude or genericDelta/specificDelta")
        for k in deltas:
            if not (isinstance(m[k], (int, float)) and not isinstance(m[k], bool)):
                v_value(m[k], mp + "." + k)  # bool excluded: a non-numeric delta must be a Value
        if "when" in m:

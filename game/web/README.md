...
Click one of your heroes → pick a skill → pick a target (enemies glow); leave a hero
unpicked to hold it, then **Resolve turn**. The AI opponent uses the engine's
`defaultPolicy`. First to 2 rounds wins. Between rounds each team currently auto-drafts
an upgrade (an interactive draft UI is the next increment).
...
- `src/view.ts` — pure DOM rendering (board / skills / log) from `(MatchState, UiState)`.
